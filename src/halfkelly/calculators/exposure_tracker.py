...
from halfkelly.instruments.forex import get_instrument
...
def create_account(account_id: str, name: str, balance: float, currency: str) -> Account:
    """
...
def create_position(
    ...
    Args:
        ...
        account: Account holding this position.
    ...
    Returns:
        Position with calculated risk_amount.
    ...
    stop_distance_pips = calculate_stop_distance_pips(entry_price, stop_loss, inst_config.pip_size)
    risk_per_lot = calculate_risk_per_lot(stop_distance_pips, inst_config.pip_value)
...
def add_position(portfolio: list[Position], position: Position) -> list[Position]:
    """
...
def remove_position(portfolio: list[Position], position_id: str) -> list[Position]:
    """
    ...
    Example:
        >>> from halfkelly.models.position import Position
        >>> portfolio = [
        ...     Position("pos1", "acc1", "EURUSD", "LONG", 0.1, 1.0, 0.9, 100.0),
        ...     Position("pos2", "acc1", "GBPUSD", "LONG", 0.1, 1.0, 0.9, 100.0),
        ... ]
...
def get_positions_by_instrument(portfolio: list[Position], instrument: str) -> list[Position]:
    """
    ...
    Example:
        >>> from halfkelly.models.position import Position
        >>> portfolio = [
        ...     Position("1", "acc1", "EURUSD", "LONG", 0.1, 1.0, 0.9, 100.0),
        ...     Position("2", "acc1", "GBPUSD", "LONG", 0.1, 1.0, 0.9, 100.0),
        ...     Position("3", "acc1", "EURUSD", "SHORT", 0.1, 1.0, 1.1, 100.0),
        ... ]
...
def get_positions_by_direction(portfolio: list[Position], direction: str) -> list[Position]:
    """
    ...
    Example:
        ...
        >>> portfolio = [
        ...     Position("1", "acc1", "EURUSD", "LONG", 0.1, 1.0, 0.9, 100.0),
        ...     Position("2", "acc1", "GBPUSD", "SHORT", 0.1, 1.0, 1.1, 100.0),
        ...     Position("3", "acc1", "USDJPY", "LONG", 0.1, 150.0, 149.0, 100.0),
        ... ]
...
def calculate_total_exposure(portfolio: list[Position], account: Account) -> ExposureSummary:
    """
    Calculate total exposure metrics for an account's positions.

    Args:
        portfolio: List of all positions.
        account: Account to calculate exposure for.

    Returns:
        ExposureSummary containing:
            ...
            - largest_position: Position with highest risk (or None if empty)
    ...
    for p in account_positions:
        risk_by_instrument[p.instrument] = risk_by_instrument.get(p.instrument, 0.0) + p.risk_amount
...
def print_exposure_summary(exposure: ExposureSummary, account: Account) -> str:
    """
    Generate a readable exposure summary string.

    Args:
        exposure: ExposureSummary from calculate_total_exposure.
        account: Account for currency display.
