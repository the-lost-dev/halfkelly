...
def main():
    ...
    risk_amount = calculate_risk_amount(account_balance=account_balance, risk_percent=risk_percent)
    print("  Formula: account_balance × (risk_percent / 100)")
    ...
    print("  Formula: position_size × risk_per_lot")
    ...
    rr_ratio = calculate_reward_risk_ratio(entry=entry_price, stop=stop_loss, target=take_profit)
    potential_reward = actual_risk * rr_ratio
    print("  Formula: |target - entry| / |entry - stop|")
    ...
    print("=" * 60)
    print("  Direction:        SHORT (stop above entry)")
